...
# =============================================================
# DATA LOADING — AUTO-DOWNLOAD OR UPLOADER FALLBACK
# =============================================================

GITHUB_BASE = "https://raw.githubusercontent.com/olistbr/brazilian-ecommerce/master"
...
def fetch_from_github(filename):
    """Try to download a CSV from the Olist GitHub repo."""
...
@st.cache_data(show_spinner=False)
def build_master(
    orders_bytes, items_bytes, payments_bytes, customers_bytes,
    reviews_bytes, products_bytes, cat_bytes
):
    """Join all Olist tables into one master DataFrame."""
    def read(b):
        return pd.read_csv(io.BytesIO(b))

    orders    = read(orders_bytes)
    items     = read(items_bytes)
    payments  = read(payments_bytes)
    customers = read(customers_bytes)
    reviews   = read(reviews_bytes)
    products  = read(products_bytes)
    cat_trans = read(cat_bytes)
    ...
    # Aggregations
    rev = payments.groupby("order_id").agg(
    ...
    items_cat = items_cat.merge(cat_trans, on="product_category_name", how="left")
    cat_agg   = items_cat.groupby("order_id")["product_category_name_english"].first().reset_index()
    cat_agg.columns = ["order_id", "category"]

    rev_scores = reviews.groupby("order_id").agg(
    ...
    df = df.merge(rev_scores, on="order_id", how="left")
...
# -----------------------------------------------------------
# Step 1: Try GitHub auto-download
# -----------------------------------------------------------
def try_github_load():
    dfs = {}
    progress = st.progress(0, text="Loading dataset from source…")
    keys = list(OLIST_FILES.keys())
    for i, (key, fname) in enumerate(OLIST_FILES.items()):
        if key == "sellers":        # sellers not needed for analysis
            dfs[key] = None
            progress.progress((i+1)/len(keys), text=f"Loading {fname}…")
            continue
        df, err = fetch_from_github(fname)
        if err:
            progress.empty()
            return None, err
        dfs[key] = df
        progress.progress((i+1)/len(keys), text=f"Loaded {fname}")
    progress.empty()
    return dfs, None

# -----------------------------------------------------------
# Step 2: Manual uploader fallback
# -----------------------------------------------------------
def uploader_ui():
    st.markdown("""
    <div class="upload-box">
        <h3 style="color:#1a3c5e; margin:0 0 8px 0">📂 Upload Olist Dataset Files</h3>
        <p style="color:#4a5568; margin:0">Running locally? Upload the 7 CSV files below to load the dashboard.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    uploaded = st.file_uploader(
        "Upload all 7 CSV files at once (select multiple)",
        type="csv",
    ...
    if not uploaded:
        st.info("Waiting for files… Download the dataset from: https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce")
        return None

    file_map = {f.name: f.read() for f in uploaded}
    result = {}
    missing = []

    for key, fname in needed.items():
        if fname in file_map:
            result[key] = file_map[fname]
        else:
    ...
    st.success("All files uploaded. Building dashboard…")
    return result
...
with st.spinner("Connecting to data source…"):
    github_dfs, github_err = try_github_load()

if github_dfs:
    # Convert DataFrames to bytes for the cached builder
    def df_to_bytes(df):
        buf = io.BytesIO()
    ...
    with st.spinner("Building master dataset…"):
        df_raw = build_master(
            df_to_bytes(github_dfs["orders"]),
            df_to_bytes(github_dfs["items"]),
            df_to_bytes(github_dfs["payments"]),
            df_to_bytes(github_dfs["customers"]),
            df_to_bytes(github_dfs["reviews"]),
            df_to_bytes(github_dfs["products"]),
            df_to_bytes(github_dfs["cat_trans"]),
        )
else:
    # Fallback: local uploader
    st.warning("Could not load data automatically. Please upload the files manually below.")
    uploaded_files = uploader_ui()
    if uploaded_files:
        with st.spinner("Building master dataset from uploaded files…"):
            df_raw = build_master(
                uploaded_files["orders"],
                uploaded_files["items"],
                uploaded_files["payments"],
                uploaded_files["customers"],
                uploaded_files["reviews"],
                uploaded_files["products"],
                uploaded_files["cat_trans"],
            )

# Stop here if data not loaded yet
if df_raw is None:
    st.stop()
